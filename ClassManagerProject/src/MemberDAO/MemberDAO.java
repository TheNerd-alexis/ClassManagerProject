...
public class MemberDAO {
	...
	private MemberDAO(Connection connection) {
		...
		eventDAO = EventDAO2.getInstance(connection);
	}
	 ...
	 *         4 = create friend failed, 5 = create event failed
	 */
	public int insertMember(String MID, String PW, String MNAME, String MCL, int PWQ, String PWA) {
		...
		try {
			...
			if (friendDAO.createFriendDb(MID) == 2)
				return 4;
			if (eventDAO.createEventDb(MID) == 2)
				return 4;
			return 1;
	 ...
	 * @return 1 = succeed, 2 = failed(same ID), 3 = delete DCH failed, <br>
	 *         4 = delete friend failed, 5 = delete event failed
	 */
		...
		if (password(PW, salt).equals(pw)) {
			...
			try {
				...
				if (friendDAO.deleteFriendDb(MID) == 2)
					return 4;
				if (eventDAO.deleteEventDb(MID) == 2)
					return 5;
				return 1;
			} catch (SQLException e) {
				// TODO Auto-generated catch block
